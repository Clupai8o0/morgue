...
import { createEncoder, recompressTree } from './image-encode.mjs'
...
// poster.png is the intermediate sharp encodes .webp/.avif FROM, and no URL
// anywhere names it: the `poster:` field below points at poster.webp, bin/grid.html
// reads that field, and bin/publish.mjs has always excluded it from the bucket for
// exactly this reason. It was still being copied in here though — 106 of them, 92MB,
// a seventh of the built tree, regenerated by any re-capture and served to nobody.
const isPosterIntermediate = (src) => path.basename(src) === 'poster.png'
...
for (const slug of slugs) {
  ...
  if (existsSync(outDir)) {
    await cp(outDir, path.join(SITE, 'media', slug), {
      recursive: true,
      force: true,
      filter: (s) => !s.includes('/frames') && publishable(s) && !isPosterIntermediate(s),
    })
    // cp merges, so filtering only stops NEW copies. Every tree built before the
    // filter existed still has one sitting there.
    await rm(path.join(SITE, 'media', slug, 'poster.png'), { force: true })
  }
...
// ─── Recompress every raster that landed in site/ ──────────────────────────
//
// Unconditional, and that is the design. Rule 11 says "optimise on the way into
// site/, never before pnpm capture" — but it was a step you had to remember, so
// it happened once, by hand, to two items. Everything else arrived at whatever
// quality the template shipped, and archives/ is the bulk of it: 602 raster
// files, 205MB, none of it ever touched.
//
// It can run with nobody reviewing it because it cannot change anything except
// bytes. Same filename, same format, same pixel dimensions, PNG stays lossless
// — see bin/image-encode.mjs, where each of those is one of rule 11's failure
// modes designed out rather than guarded against. A rename would 404 a computed
// path with no string to rewrite; a resize would break anything whose geometry
// is load-bearing; a format change breaks the MIME. None is reachable from here.
//
// site/ is regenerable, so this is the one tree where in-place is free. items/
// is paid source and is never touched by a build — `pnpm optimise --in-place`
// remains the only thing that writes there, and it still costs four flags.
//
// Escape hatch for bisecting a rendering bug, not for routine use.
const OPTIMISE =
  !process.argv.includes('--no-optimise') && process.env.MORGUE_NO_OPTIMISE !== '1'
if (OPTIMISE) {
  // Same cache directory bin/optimise.mjs uses, and the same key, so the two
  // share work instead of each paying for it.
  const { encode } = createEncoder({
    cacheDir: path.join(ROOT, 'out', '.optimise-cache'),
    persist: true,
  })
  const totals = { files: 0, changed: 0, before: 0, after: 0, grew: 0 }
  for (const sub of ['item', 'archive']) {
    const r = await recompressTree(path.join(SITE, sub), { encode })
    for (const k of Object.keys(totals)) totals[k] += r[k]
  }
  if (totals.files) {
    const pct = totals.before ? ((totals.after - totals.before) / totals.before) * 100 : 0
    console.log(
      `  images: ${totals.changed}/${totals.files} recompressed, ` +
        `${(totals.before / 1048576).toFixed(1)}MB → ${(totals.after / 1048576).toFixed(1)}MB ` +
        `(${pct.toFixed(1)}%)${totals.grew ? `, ${totals.grew} already optimal` : ''}`,
    )
  }
} else {
  console.log('  images: --no-optimise — site/ carries the source bytes verbatim')
}
